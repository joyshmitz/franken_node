"""Unit tests for scripts/check_staking_slashing.py (bd-26mk)."""
...
import check_staking_slashing as checker
...
class TestRunAllStructure(unittest.TestCase):
    ...
    def test_title(self):
        result = checker.run_all()
        self.assertEqual(result["title"], "Security Staking and Slashing Framework")
...
class TestCoreTypes(unittest.TestCase):
    ...
    def test_type_count(self):
        self.assertEqual(len(checker.CORE_TYPES), 12)
...
class TestBTreeMap(unittest.TestCase):
    def test_btreemap_usage(self):
        result = checker.run_all()
        check = next(c for c in result["checks"] if c["name"] == "BTREEMAP_USAGE")
        self.assertTrue(check["passed"], check["detail"])
...
class TestJsonOutput(unittest.TestCase):
    ...
    def test_cli_json(self):
        proc = subprocess.run(
            [sys.executable,
             str(ROOT / "scripts" / "check_staking_slashing.py"), "--json"],
            capture_output=True, text=True,
        )
        data = json.loads(proc.stdout)
        self.assertEqual(data["bead_id"], "bd-26mk")
        self.assertIn("checks", data)

    def test_cli_self_test(self):
        proc = subprocess.run(
            [sys.executable,
             str(ROOT / "scripts" / "check_staking_slashing.py"), "--self-test"],
            capture_output=True, text=True,
        )
