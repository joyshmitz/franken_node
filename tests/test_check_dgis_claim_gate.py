"""Unit tests for scripts/check_dgis_claim_gate.py (bd-38yt)."""
...
spec = importlib.util.spec_from_file_location(
    "check_dgis_claim_gate",
    ROOT / "scripts" / "check_dgis_claim_gate.py",
)
...
class TestDgisClaimGate(unittest.TestCase):
    def test_repo_artifacts_pass(self) -> None:
        report = mod.run_checks()
        self.assertEqual(report["bead_id"], "bd-38yt")
        self.assertEqual(report["verdict"], "PASS")

    def test_tampered_signature_fails(self) -> None:
        with tempfile.TemporaryDirectory(prefix="bd-38yt-test-") as tmp:
            base = Path(tmp)
    ...
    def test_missing_evidence_fails(self) -> None:
        ...
        self.assertTrue(
            any(
                check["check"] == "claim DGIS-CLAIM-004" and not check["pass"]
                for check in report["checks"]
            )
        )
    ...
    def test_p95_budget_breach_fails(self) -> None:
        ...
        self.assertTrue(
            any(
                check["check"] == "claim DGIS-CLAIM-002" and not check["pass"]
                for check in report["checks"]
            )
        )
    ...
    def test_p99_budget_breach_fails(self) -> None:
        ...
        self.assertTrue(
            any(
                check["check"] == "claim DGIS-CLAIM-003" and not check["pass"]
                for check in report["checks"]
            )
        )

    def test_signed_evidence_shortfall_fails(self) -> None:
        with tempfile.TemporaryDirectory(prefix="bd-38yt-test-") as tmp:
            base = Path(tmp)
            spec_path, report_path, _ = mod._signed_report_fixture(
