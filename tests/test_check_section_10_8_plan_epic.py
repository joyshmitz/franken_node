"""Tests for scripts/check_section_10_8_plan_epic.py (bd-c4f)."""
...
SCRIPT = os.path.join(ROOT, "scripts", "check_section_10_8_plan_epic.py")

spec = importlib.util.spec_from_file_location("check_section_10_8_plan_epic", SCRIPT)
...
class TestConstants:
    def test_identity_constants(self):
        assert mod.BEAD_ID == "bd-c4f"
        assert mod.SECTION == "10.8"
        assert mod.SECTION_GATE_ID == "bd-1fi2"

    def test_impl_bead_count(self):
        assert len(mod.SECTION_IMPL_BEADS) == 6
...
class TestJsonOutput:
    def test_cli_json_output(self):
        ...
        assert data["bead_id"] == "bd-c4f"
        assert data["section"] == "10.8"
...
class TestRunAll:
    ...
    def test_metrics_are_coherent(self, result):
        ...
        assert metrics["cross_epic_dependency_count"] >= 3
        assert metrics["section_contract_spec_count"] >= 6
        assert metrics["section_impl_bead_count"] == 6


class TestHelpers:
    def test_load_json_none_for_missing(self):
        missing_path = os.path.join(ROOT, "artifacts", "section_10_8", "bd-does-not-exist", "missing.json")
