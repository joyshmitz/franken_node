"""Unit tests for scripts/check_vef_claim_gate.py (bd-3lzk)."""
...
spec = importlib.util.spec_from_file_location(
    "check_vef_claim_gate",
    ROOT / "scripts" / "check_vef_claim_gate.py",
)
...
class TestVefClaimGate(unittest.TestCase):
    def test_repo_artifacts_pass(self) -> None:
        report = mod.run_checks()
        self.assertEqual(report["bead_id"], "bd-3lzk")
        self.assertEqual(report["verdict"], "PASS")

    def test_tampered_signature_fails(self) -> None:
        with tempfile.TemporaryDirectory(prefix="bd-3lzk-test-") as tmp:
            base = Path(tmp)
    ...
    def test_missing_evidence_fails(self) -> None:
        ...
        self.assertTrue(
            any(
                check["check"] == "claim VEF-CLAIM-003" and not check["pass"]
                for check in report["checks"]
            )
        )
