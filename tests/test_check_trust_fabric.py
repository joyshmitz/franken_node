"""Unit tests for scripts/check_trust_fabric.py (bd-5si)."""
...
import check_trust_fabric as checker
...
class TestRunAllStructure(unittest.TestCase):
    ...
    def test_bead_id(self):
        result = checker.run_all()
        self.assertEqual(result["bead_id"], "bd-5si")
    ...
    def test_title(self):
        result = checker.run_all()
        self.assertEqual(result["title"], "Trust Fabric Convergence Protocol")
...
class TestPolicyChecks(unittest.TestCase):
    ...
    def test_policy_topics(self):
        ...
        self.assertTrue(len(topic_checks) > 0, "No policy topic checks found")
...
class TestConstants(unittest.TestCase):
    ...
    def test_type_count(self):
        self.assertEqual(len(checker.REQUIRED_TYPES), 7)
...
class TestJsonOutput(unittest.TestCase):
    ...
    def test_cli_json(self):
        proc = subprocess.run(
            [sys.executable,
             str(ROOT / "scripts" / "check_trust_fabric.py"), "--json"],
            capture_output=True, text=True,
        )
        data = json.loads(proc.stdout)
        self.assertEqual(data["bead_id"], "bd-5si")
        self.assertIn("checks", data)

    def test_cli_self_test(self):
        proc = subprocess.run(
            [sys.executable,
             str(ROOT / "scripts" / "check_trust_fabric.py"), "--self-test"],
            capture_output=True, text=True,
        )
...
class TestRustTestCategories(unittest.TestCase):
    ...
    def test_state_tests(self):
        result = checker.run_all()
        check = next(c for c in result["checks"] if c["name"] == "rust_state_tests")
        self.assertTrue(check["passed"], check["detail"])

    def test_delta_tests(self):
        result = checker.run_all()
        check = next(c for c in result["checks"] if c["name"] == "rust_delta_tests")
        self.assertTrue(check["passed"], check["detail"])

    def test_gossip_tests(self):
        result = checker.run_all()
        check = next(c for c in result["checks"] if c["name"] == "rust_gossip_tests")
        self.assertTrue(check["passed"], check["detail"])
    ...
    def test_convergence_tests(self):
        result = checker.run_all()
        check = next(c for c in result["checks"] if c["name"] == "rust_convergence_tests")
        self.assertTrue(check["passed"], check["detail"])

    def test_anti_entropy_tests(self):
        result = checker.run_all()
        check = next(c for c in result["checks"] if c["name"] == "rust_anti_entropy_tests")
        self.assertTrue(check["passed"], check["detail"])

    def test_partition_tests(self):
        result = checker.run_all()
        check = next(c for c in result["checks"] if c["name"] == "rust_partition_tests")
        self.assertTrue(check["passed"], check["detail"])
    ...
    def test_event_tests(self):
        result = checker.run_all()
        check = next(c for c in result["checks"] if c["name"] == "rust_event_tests")
        self.assertTrue(check["passed"], check["detail"])
