"""Unit tests for scripts/check_atc_participation.py."""
...
import check_atc_participation as mod


class TestConstants(unittest.TestCase):
    def test_required_type_count(self):
        self.assertEqual(len(mod.REQUIRED_TYPES), 10)
    ...
    def test_attestation_level_count(self):
        self.assertEqual(len(mod.ATTESTATION_LEVELS), 4)

    def test_weight_component_count(self):
        self.assertEqual(len(mod.WEIGHT_COMPONENTS), 5)
...
class TestRunAll(unittest.TestCase):
    def test_structure(self):
        ...
        for key in [
            ...
            "required_types", "event_codes", "invariants", "checks",
        ]:
            self.assertIn(key, result)

    def test_identity(self):
        result = mod.run_all()
        self.assertEqual(result["bead_id"], "bd-2yvw")
    ...
    def test_json_serializable(self):
        ...
        self.assertEqual(parsed["bead_id"], "bd-2yvw")
...
class TestKeyChecks(unittest.TestCase):
    ...
    def test_main_registration(self):
        checks = {c["check"]: c for c in mod.run_all_checks()}
        self.assertTrue(checks["main_federation_registration"]["pass"])

    def test_sybil_detection(self):
        checks = {c["check"]: c for c in mod.run_all_checks()}
        self.assertTrue(checks["sybil_detection"]["pass"])
    ...
    def test_weight_computation(self):
        checks = {c["check"]: c for c in mod.run_all_checks()}
        self.assertTrue(checks["weight_computation"]["pass"])
