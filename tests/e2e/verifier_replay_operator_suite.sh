...
TRACE_ID="${TRACE_ID:-trace-bd-1z5a-operator-e2e}"
...
run_stage() {
  ...
  set +e
  bash -lc "cd '${ROOT_DIR}' && rch exec -- ${effective_command}" >"${stdout_path}" 2>"${stderr_path}"
  exit_code=$?
  set -e
...
build_ids = [row["build_id"] for row in stage_results if row.get("build_id")]

verdict = "PASS"
if fail_count > 0 or missing_stage_ids or missing_event_codes or len(trace_ids) != 1:
    verdict = "FAIL"
...
with summary_md_path.open("w", encoding="utf-8") as fh:
    ...
    fh.write(f"- Build IDs: `{', '.join(build_ids) if build_ids else 'none-detected'}`\n")
    fh.write("\n")
    fh.write("| Stage | Event | Decision | Reason | Status | Exit | Build ID |\n")
    fh.write("|---|---|---|---|---|---:|---|\n")
    for row in stage_results:
        fh.write(
            "| {stage_id} | `{event_code}` | `{decision}` | `{reason_code}` | {status} | {exit_code} | `{build_id}` |\n".format(
