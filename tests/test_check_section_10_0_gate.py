"""Unit tests for scripts/check_section_10_0_gate.py (bd-3qsp)."""
...
SCRIPT = ROOT / "scripts" / "check_section_10_0_gate.py"

spec = importlib.util.spec_from_file_location("check_section_10_0_gate", SCRIPT)
...
class TestRunAllShape(unittest.TestCase):
    def test_run_all_shape(self) -> None:
        result = mod.run_all()
        self.assertEqual(result["bead_id"], "bd-3qsp")
        self.assertEqual(result["section"], "10.0")
    ...
    def test_section_beads_count(self) -> None:
        result = mod.run_all()
        self.assertEqual(len(result["section_beads"]), 10)
...
class TestCli(unittest.TestCase):
    ...
    def test_self_test_json_cli(self) -> None:
        ...
        self.assertEqual(parsed["bead_id"], "bd-3qsp")
...
class TestConstants(unittest.TestCase):
    def test_section_bead_count(self) -> None:
        self.assertEqual(len(mod.SECTION_BEADS), 10)

    def test_domain_group_count(self) -> None:
        self.assertEqual(len(mod.DOMAIN_GROUPS), 10)
    ...
    def test_gate_bead_id(self) -> None:
        self.assertEqual(mod.GATE_BEAD, "bd-3qsp")
...
class TestEvidencePass(unittest.TestCase):
    ...
    def test_verification_results_python_checker(self) -> None:
        self.assertTrue(mod._evidence_pass({
            "verification_results": {
