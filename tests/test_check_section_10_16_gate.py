"""Unit tests for scripts/check_section_10_16_gate.py."""
...
import check_section_10_16_gate as mod


class TestConstants(unittest.TestCase):
    def test_section_beads_count(self):
        self.assertEqual(len(mod.SECTION_BEADS), 17)

    def test_bead_ids(self):
        ids = [b[0] for b in mod.SECTION_BEADS]
        for expected in [
            "bd-2owx", "bd-28ld", "bd-34ll", "bd-1xtf", "bd-1719",
            "bd-1a1j", "bd-2tua", "bd-26ux", "bd-bt82", "bd-1v65",
            "bd-3ndj", "bd-2f5l", "bd-159q", "bd-2ji2", "bd-35l5",
            "bd-3u2o", "bd-8l9k",
        ]:
    ...
    def test_bead_id_constant(self):
        self.assertEqual(mod.BEAD, "bd-10g0")

    def test_section_constant(self):
        self.assertEqual(mod.SECTION, "10.16")
...
class TestRunAll(unittest.TestCase):
    ...
    def test_bead_id(self):
        result = mod.run_all()
        self.assertEqual(result["bead_id"], "bd-10g0")
    ...
    def test_section_beads_list(self):
        result = mod.run_all()
        self.assertEqual(len(result["section_beads"]), 17)
        self.assertIn("bd-2owx", result["section_beads"])
        self.assertIn("bd-8l9k", result["section_beads"])

    def test_checks_list(self):
        result = mod.run_all()
        self.assertIsInstance(result["checks"], list)
        self.assertGreaterEqual(len(result["checks"]), 45)
...
class TestJsonOutput(unittest.TestCase):
    def test_serializable(self):
        ...
        self.assertEqual(parsed["bead_id"], "bd-10g0")
...
class TestSubstrateCoverage(unittest.TestCase):
    def test_frankentui(self):
        results = mod.run_all_checks()
        check = next(r for r in results if r["check"] == "substrate_frankentui_coverage")
        self.assertTrue(check["pass"])

    def test_frankensqlite(self):
        results = mod.run_all_checks()
        check = next(r for r in results if r["check"] == "substrate_frankensqlite_coverage")
        self.assertTrue(check["pass"])

    def test_sqlmodel_rust(self):
        results = mod.run_all_checks()
        check = next(r for r in results if r["check"] == "substrate_sqlmodel_rust_coverage")
        self.assertTrue(check["pass"])

    def test_fastapi_rust(self):
        results = mod.run_all_checks()
        check = next(r for r in results if r["check"] == "substrate_fastapi_rust_coverage")
        self.assertTrue(check["pass"])
