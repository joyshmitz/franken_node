...
pub const REQUIRED_ARTIFACT_TYPES: &[&str] = &[
    "report_pdf", "data_bundle", "reproduction_script",
    "verification_evidence", "methodology_doc",
];
...
impl ReportType {
    pub fn all() -> &'static [ReportType] {
        &[Self::TechnicalAnalysis, Self::SecurityAssessment, Self::PerformanceBenchmark,
          Self::ComplianceReport, Self::IncidentPostmortem]
    }
...
impl Default for ReportOutputContract {
    fn default() -> Self {
        Self { contract_version: CONTRACT_VERSION.to_string(), bundles: BTreeMap::new(), audit_log: Vec::new() }
    }
}

impl ReportOutputContract {
    pub fn create_bundle(&mut self, mut bundle: ReportBundle, trace_id: &str) -> Result<String, String> {
        ...
        for art in &bundle.artifacts {
            if art.content_hash.len() != 64 || !art.content_hash.chars().all(|c| c.is_ascii_hexdigit()) {
                self.log(event_codes::ROC_ERR_HASH_MISMATCH, trace_id, serde_json::json!({"artifact_type": &art.artifact_type}));
        ...
        // Check completeness
        let present_types: Vec<&str> = bundle.artifacts.iter().map(|a| a.artifact_type.as_str()).collect();
        let missing: Vec<&&str> = REQUIRED_ARTIFACT_TYPES.iter().filter(|t| !present_types.contains(*t)).collect();
        bundle.is_complete = missing.is_empty();

        if !bundle.is_complete {
            self.log(event_codes::ROC_ERR_INCOMPLETE, trace_id, serde_json::json!({"missing": missing.len()}));
        }

        self.log(event_codes::ROC_COMPLETENESS_CHECKED, trace_id, serde_json::json!({"complete": bundle.is_complete}));
        self.log(event_codes::ROC_REPRODUCIBILITY_VERIFIED, trace_id, serde_json::json!({"command": &bundle.reproduction_command}));
        ...
        self.log(event_codes::ROC_VERSION_EMBEDDED, trace_id, serde_json::json!({"version": &bundle.contract_version}));
        self.bundles.insert(bid.clone(), bundle);
        self.log(event_codes::ROC_BUNDLE_CREATED, trace_id, serde_json::json!({"bundle_id": &bid}));
    ...
    pub fn generate_catalog(&mut self, trace_id: &str) -> OutputCatalog {
        ...
        for b in self.bundles.values() {
            *by_type.entry(b.report_type.label().to_string()).or_insert(0) += 1;
            if b.is_complete { complete += 1; }
        }
        ...
        self.log(event_codes::ROC_CATALOG_GENERATED, trace_id, serde_json::json!({"total": self.bundles.len(), "complete": complete}));

        OutputCatalog {
            catalog_id: Uuid::now_v7().to_string(), timestamp: Utc::now().to_rfc3339(),
            contract_version: self.contract_version.clone(), total_bundles: self.bundles.len(),
            complete_bundles: complete, by_type, content_hash,
        }
    }

    pub fn bundles(&self) -> &BTreeMap<String, ReportBundle> { &self.bundles }
    pub fn audit_log(&self) -> &[RocAuditRecord] { &self.audit_log }
...
mod tests {
    ...
    fn sample_artifact(atype: &str) -> ArtifactEntry {
        ArtifactEntry { artifact_type: atype.to_string(), path: format!("/out/{}", atype), content_hash: "a".repeat(64), size_bytes: 1024 }
    }
    fn complete_bundle(id: &str) -> ReportBundle {
        ReportBundle {
            bundle_id: id.to_string(), report_type: ReportType::TechnicalAnalysis,
            title: "Test Report".to_string(),
            artifacts: REQUIRED_ARTIFACT_TYPES.iter().map(|t| sample_artifact(t)).collect(),
            reproduction_command: "make reproduce".to_string(),
            contract_version: String::new(), is_complete: false,
            bundle_hash: String::new(), created_at: String::new(),
        }
    }
    ...
    #[test] fn five_report_types() { assert_eq!(ReportType::all().len(), 5); }
    #[test] fn five_required_artifact_types() { assert_eq!(REQUIRED_ARTIFACT_TYPES.len(), 5); }
    #[test] fn create_complete_bundle() {
    ...
    #[test] fn create_partial_bundle_incomplete() {
        let mut e = ReportOutputContract::default();
        e.create_bundle(partial_bundle("b-1"), &trace()).unwrap();
        assert!(!e.bundles().get("b-1").unwrap().is_complete);
    }
    #[test] fn create_empty_repro_command_fails() {
        let mut e = ReportOutputContract::default();
        let mut b = complete_bundle("b-1"); b.reproduction_command = String::new();
        assert!(e.create_bundle(b, &trace()).is_err());
    }
    #[test] fn create_bad_hash_fails() {
        let mut e = ReportOutputContract::default();
        let mut b = complete_bundle("b-1");
        b.artifacts[0].content_hash = "short".to_string();
        assert!(e.create_bundle(b, &trace()).is_err());
    }
    #[test] fn create_sets_version() {
        let mut e = ReportOutputContract::default();
        e.create_bundle(complete_bundle("b-1"), &trace()).unwrap();
        assert_eq!(e.bundles().get("b-1").unwrap().contract_version, CONTRACT_VERSION);
    }
    #[test] fn create_sets_hash() {
        let mut e = ReportOutputContract::default();
        e.create_bundle(complete_bundle("b-1"), &trace()).unwrap();
        assert_eq!(e.bundles().get("b-1").unwrap().bundle_hash.len(), 64);
    }
    #[test] fn catalog_empty() {
        let mut e = ReportOutputContract::default();
        let c = e.generate_catalog(&trace());
        assert_eq!(c.total_bundles, 0);
    }
    #[test] fn catalog_counts() {
        let mut e = ReportOutputContract::default();
        e.create_bundle(complete_bundle("b-1"), &trace()).unwrap();
