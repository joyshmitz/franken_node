...
impl RemoteCapabilityTestHarness {
    async fn new() -> Self {
        ...
        let root_secret =
            frankenengine_node::security::epoch_scoped_keys::RootSecret::generate_test_key();
    ...
    async fn enforce_capability_with_timing(
        &mut self,
        client_id: &str,
        capability: &str,
        payload: &[u8],
        ...
        let enforcement_result = self
            .bulkhead
            .enforce_capability(client_id, capability, payload)
            .await;
    ...
    async fn concurrent_capability_stress(
        &mut self,
        client_count: usize,
        ...
        for i in 0..client_count {
            ...
            let handle = tokio::spawn(async move {
                ...
                for req_idx in 0..requests_per_client {
                    ...
                    let payload =
                        format!("{{\"request\": {}, \"data\": \"test-payload\"}}", req_idx)
                            .into_bytes();
    ...
    async fn test_adversarial_boundary_conditions(&mut self) -> Vec<(String, bool)> {
        let adversarial_test_cases = vec![
            ...
            (
                "special_chars_client",
                "client\0\n\r\t",
                "test.capability",
                b"payload",
            ),
            (
                "special_chars_capability",
                "client-1",
                "test\0\n.capability",
                b"payload",
            ),
        ...
        for (test_name, client_id, capability, payload) in adversarial_test_cases {
            ...
            let success = match test_name {
                ...
                name if name.contains("empty")
                    || name.contains("path_traversal")
                    || name.contains("injection") =>
                {
                    result.is_err()
                }
                // These should succeed or fail gracefully
                _ => result.is_ok() || result.is_err(),
            };
    ...
    fn export_performance_summary(&self) -> serde_json::Value {
        ...
        let max_concurrent = self
            .operation_logs
            .iter()
            .map(|log| log.concurrent_requests)
            .max()
            .unwrap_or(0);
...
async fn test_remote_capability_enforcement_real_service_integration() {
    ...
    let enforcement_duration = harness
        .enforce_capability_with_timing("test-client-001", "test.basic.capability", b"test-payload")
        .await
        .expect("Basic capability enforcement should succeed");

    // Verify real timing (not mocked instant responses)
    assert!(
        enforcement_duration >= Duration::from_micros(10),
        "Real enforcement should take measurable time"
    );
    assert!(
        enforcement_duration <= Duration::from_secs(5),
        "Enforcement should complete promptly"
    );
...
async fn test_concurrent_capability_enforcement_under_stress() {
    ...
    let stress_results = harness
        .concurrent_capability_stress(CONCURRENT_CLIENTS, REQUESTS_PER_CLIENT)
        .await;
    ...
    assert!(
        success_rate >= 0.8,
        "Success rate under load should be >= 80%, got {:.1}%",
        success_rate * 100.0
    );
...
async fn test_capability_enforcement_adversarial_boundary_conditions() {
    ...
    for (test_name, success) in &boundary_results {
        if test_name.contains("empty")
            || test_name.contains("path_traversal")
            || test_name.contains("injection")
        {
    ...
    assert_eq!(
        security_boundaries_held, total_security_tests,
        "All security boundaries must hold: {}/{} passed",
        security_boundaries_held, total_security_tests
    );
...
async fn test_capability_round_trip_determinism() {
    ...
    assert!(
        is_deterministic,
        "Capability enforcement must be deterministic across multiple invocations"
    );
