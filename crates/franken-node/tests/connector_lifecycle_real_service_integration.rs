...
impl ConnectorLifecycleTestHarness {
    ...
    async fn transition_with_timing(
        &mut self,
        connector_id: &str,
    ...
    async fn parallel_transition_stress_test(
        &mut self,
        connector_count: usize,
        transitions_per_connector: usize,
    ...
    fn export_test_summary(&self) -> serde_json::Value {
        ...
        let successful_transitions = self
            .phase_logs
            .iter()
            .filter(|log| log.error.is_none())
            .count();
        let failed_transitions = self
            .phase_logs
            .iter()
            .filter(|log| log.error.is_some())
            .count();
...
async fn test_connector_lifecycle_happy_path_with_real_services() {
    ...
    for (i, &target_state) in states.iter().enumerate().skip(1) {
        let duration = harness
            .transition_with_timing(connector_id, target_state)
            .await
        ...
        assert!(
            ...
            "Transition to {:?} was suspiciously fast: {:?}",
            target_state,
            duration
        );
        assert!(
            duration <= Duration::from_secs(5),
            "Transition to {:?} took too long: {:?}",
            target_state,
            duration
        );
    }
...
async fn test_connector_cancellation_protocol_three_phase() {
    ...
    harness
        .transition_with_timing(connector_id, ConnectorState::Verified)
        .await
        .unwrap();
    harness
        .transition_with_timing(connector_id, ConnectorState::Installed)
        .await
        .unwrap();
    harness
        .transition_with_timing(connector_id, ConnectorState::Configured)
        .await
        .unwrap();
    harness
        .transition_with_timing(connector_id, ConnectorState::Active)
        .await
        .unwrap();
    ...
    let request_duration = harness
        .transition_with_timing(connector_id, ConnectorState::Cancelling)
        .await
    ...
    assert!(
        drain_result.is_ok(),
        "Drain phase should complete within timeout"
    );
    let drain_duration = drain_start.elapsed();

    // Phase 3: FINALIZE - Transition to stopped
    let finalize_duration = harness
        .transition_with_timing(connector_id, ConnectorState::Stopped)
        .await
    ...
    assert!(
        total_cancellation >= Duration::from_millis(10),
        "Cancellation should not be instantaneous"
    );
    assert!(
        total_cancellation <= Duration::from_secs(15),
        "Cancellation should complete within reasonable time"
    );
...
async fn test_connector_lease_conflict_resolution() {
    ...
    let lease1_result = harness
        .lease_service
        .acquire_exclusive_lease(connector_id, "owner1", Duration::from_secs(30))
        .await;
    assert!(lease1_result.is_ok(), "First lease should succeed");

    let lease2_result = harness
        .lease_service
        .acquire_exclusive_lease(connector_id, "owner2", Duration::from_secs(30))
        .await;
    assert!(
        matches!(lease2_result, Err(LeaseConflictError::AlreadyLeased { .. })),
    ...
    let lease3_result = harness
        .lease_service
        .acquire_exclusive_lease(connector_id, "owner3", Duration::from_millis(50))
        .await;
    // Should still conflict with unexpired lease1
    assert!(
        lease3_result.is_err(),
        "Third lease should fail - lease1 still active"
    );
...
async fn test_concurrent_lifecycle_transitions_race_conditions() {
    ...
    assert!(
        success_rate >= 0.8,
        "Success rate too low: {:.2}%, expected >= 80%",
        success_rate * 100.0
    );
...
async fn test_health_gate_integration_with_lifecycle() {
    ...
    harness
        ...
        .set_health(
            connector_id,
            HealthStatus::Unhealthy("Mock service down".to_string()),
        )
        .await;

    let activation_result = harness
        .transition_with_timing(connector_id, ConnectorState::Active)
        .await;
    assert!(
        activation_result.is_err(),
        "Activation should fail when health gate reports unhealthy"
    );

    // Test successful activation after health recovery
    harness
        .health_gate
        .set_health(connector_id, HealthStatus::Healthy)
        .await;
    ...
    assert!(
        ...
        "Activation should succeed after health recovery"
    );
...
async fn test_lifecycle_error_boundary_conditions() {
    ...
    let test_cases = vec![
        (
            "illegal_transition",
            "connector-illegal-001",
            ConnectorState::Discovered,
            ConnectorState::Active,
        ), // Skip intermediate states
        (
            "double_transition",
            "connector-double-001",
            ConnectorState::Discovered,
            ConnectorState::Verified,
        ), // Will try twice
        (
        ...
        match test_name {
            ...
            "double_transition" => {
                ...
                assert!(
                    ...
                    "Second transition should handle gracefully"
                );
            }
            "empty_id" => {
            ...
            "very_long_id" => {
                ...
                assert!(
                    ...
                    "Very long ID should not cause panic"
                );
            }
            _ => {}
        }
