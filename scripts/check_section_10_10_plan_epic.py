#!/usr/bin/env python3
"""Verification script for bd-1hf (Section 10.10 integration epic).

Ensures the plan-epic closure conditions are satisfied:
- All child dependencies are closed.
- Section verification gate evidence (bd-1jjq) is PASS.
- Section 10.10 implementation evidence/spec artifacts are present.
"""
...
def run_all() -> dict[str, Any]:
    ...
    checks.append(
        _check(
            ...
            epic is not None,
            "br show bd-1hf readable" if epic is not None else "unable to load br show bd-1hf --json",
        )
    )
    if epic is None:
        return {
    ...
    checks.append(
        _check(
            "epic_section_label",
            "section-10-10" in (epic.get("labels") or []),
    ...
    section_gate_evidence = ROOT / "artifacts" / "section_10_10" / SECTION_GATE_ID / "verification_evidence.json"
    section_gate_summary = ROOT / "artifacts" / "section_10_10" / SECTION_GATE_ID / "verification_summary.md"
    checks.append(
        _check(
    ...
    spec_dir = ROOT / "docs" / "specs" / "section_10_10"
    ...
    for bead_id in SECTION_IMPL_BEADS:
        evidence_path = ROOT / "artifacts" / "section_10_10" / bead_id / "verification_evidence.json"
        summary_path = ROOT / "artifacts" / "section_10_10" / bead_id / "verification_summary.md"
...
def main() -> None:
    logger = configure_test_logging("check_section_10_10_plan_epic")
